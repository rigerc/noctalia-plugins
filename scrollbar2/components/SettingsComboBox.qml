import QtQuick
import QtQuick.Layouts
import qs.Commons
import qs.Widgets

NComboBox {
    id: root

    property var rootSettings: null
    property string settingPath: ""
    property var modelSource: null
    property var visibilityConditions: []
    property var disabledConditions: []

    readonly property bool _isDisabled: rootSettings ? rootSettings.isDisabledByConditions(disabledConditions) : false

    Layout.fillWidth: true
    enabled: !_isDisabled
    opacity: _isDisabled ? 0.4 : 1.0
    visible: rootSettings ? rootSettings.isVisibleByConditions(visibilityConditions) : true

    model: modelSource
    currentKey: rootSettings ? (rootSettings.getPath(settingPath) ?? "") : ""
    defaultValue: rootSettings ? rootSettings.getDefault(settingPath) : ""

    onSelected: key => {
        if (rootSettings) rootSettings.setPath(settingPath, key);
    }
}
