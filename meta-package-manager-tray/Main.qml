...
Item {
    ...
    Process {
        ...
        stdout: StdioCollector {
            id: availabilityStdout
        }
        stderr: StdioCollector {
            id: availabilityStderr
        }

        onExited: function (exitCode, exitStatus) {
            root.handleAvailabilityResult(exitCode, String(availabilityStdout.text || ""), String(availabilityStderr.text || ""));
        }
    }
    ...
    Process {
        ...
        onExited: function (exitCode, exitStatus) {
            root.handleManagersResult(exitCode, String(managersStdout.text || ""), String(managersStderr.text || ""));
        }
    }
    ...
    Process {
        ...
        onExited: function (exitCode, exitStatus) {
            root.handleOutdatedResult(exitCode, String(outdatedStdout.text || ""), String(outdatedStderr.text || ""));
        }
    }
    ...
    function refresh(showAvailabilityToast, reason) {
        ...
        availabilityProcess.command = ["sh", "-c", "mpm_path=$(command -v mpm 2>/dev/null) || exit 127; printf '%s\\n' \"$mpm_path\"; mpm --version"];
    ...
    function upgrade() {
        ...
        if (root.activeManagerIds.length === 0) {
            ToastService.showWarning(pluginApi?.tr("plugin.name"), pluginApi?.tr("toast.noActiveManagers"), "alert-triangle");
            return;
        }

        if (root.terminalCommand.trim() === "") {
            ToastService.showWarning(pluginApi?.tr("plugin.name"), pluginApi?.tr("toast.terminalMissing"), "alert-triangle");
            return;
        }
    ...
    function handleAvailabilityResult(exitCode, stdoutText, stderrText) {
        ...
        if (exitCode !== 0) {
            ...
            root.mpmErrorMessage = isBroken ? pluginApi?.tr("status.mpmBroken") : pluginApi?.tr("status.mpmMissing");
        ...
        managersProcess.command = ["mpm", "--output-format", "json", "--no-color", "--verbosity", "ERROR", "managers"];
    ...
    function handleManagersResult(exitCode, stdoutText, stderrText) {
        ...
        root.activeManagerIds = root.availableManagers.filter(function (manager) {
    ...
    function setCommandFailure(kind, detail) {
        ...
        var translatedMessage = kind.indexOf("managers") === 0 ? pluginApi?.tr("status.managersFailed") : pluginApi?.tr("status.outdatedFailed");
    ...
    function finalizeSuccessfulRefresh(totalCount) {
        var now = Date.now();
        var shouldNotify = root.enableNotifications && root.hasCompletedInitialSuccessfulRefresh && totalCount > root.previousSuccessfulUpdateCount;
        ...
        if (shouldNotify) {
            ToastService.showNotice(pluginApi?.tr("plugin.name"), pluginApi?.tr("toast.updatesAvailable", {
                "count": totalCount
            }), "package");
        }
    }
    ...
    function showAvailabilityWarning(force) {
        ...
        if (!force) {
            if (!isBroken && root.hasShownMissingMpmToast)
                return;
            if (isBroken && root.hasShownBrokenMpmToast)
                return;
        }

        if (isBroken)
            root.hasShownBrokenMpmToast = true;
        else
            root.hasShownMissingMpmToast = true;

        ToastService.showWarning(pluginApi?.tr("plugin.name"), isBroken ? pluginApi?.tr("toast.mpmBroken") : pluginApi?.tr("toast.mpmMissing"), "alert-triangle");
    }
    ...
    function buildOutdatedCommand(ids) {
        return ["mpm", "--output-format", "json", "--no-color", "--no-stats", "--verbosity", "ERROR", "--timeout", "30"].concat(root.buildManagerSelectorFlags(ids)).concat(["outdated"]);
    }

    function buildUpgradeCommand(ids) {
        return ["mpm", "--verbosity", "INFO"].concat(root.buildManagerSelectorFlags(ids)).concat(["upgrade"]);
    }
    ...
    function formatTimestamp(value) {
        if (!value || value <= 0)
            return pluginApi?.tr("common.never");
