...
Item {
    ...
    readonly property int summaryColumns: summaryGrid.width >= Math.round(420 * Style.uiScaleRatio) ? 3 : (summaryGrid.width >= Math.round(280 * Style.uiScaleRatio) ? 2 : 1)
                            ...
                            ColumnLayout {
                                ...
                                RowLayout {
                                    ...
                                    Rectangle {
                                        ...
                                        color: modelData.errorCount > 0 ? Qt.alpha(Color.resolveColorKey("destructive"), 0.14) : Qt.alpha(Color.mSecondary, 0.14)
    ...
    function shouldShowStateCard() {
        if (mainInstance?.isRefreshing ?? false)
            return true;
        if (!(mainInstance?.mpmAvailable ?? false))
            return true;
        if ((mainInstance?.activeManagerIds?.length ?? 0) === 0)
            return true;
        if ((mainInstance?.hasError ?? false) && root.visibleResults.length === 0)
            return true;
        if ((mainInstance?.totalUpdateCount ?? 0) === 0 && !(mainInstance?.hasError ?? false))
            return true;
        return false;
    }
    ...
    function stateTitle() {
        if (mainInstance?.isRefreshing ?? false)
            return pluginApi?.tr("panel.state.checkingTitle");
        if (!(mainInstance?.mpmAvailable ?? false))
            return pluginApi?.tr("panel.state.unavailableTitle");
        if ((mainInstance?.activeManagerIds?.length ?? 0) === 0)
            return pluginApi?.tr("panel.state.noManagersTitle");
        if ((mainInstance?.hasError ?? false) && root.visibleResults.length === 0)
            return pluginApi?.tr("panel.state.errorTitle");
        return pluginApi?.tr("panel.state.noUpdatesTitle");
    }

    function stateMessage() {
        if (mainInstance?.isRefreshing ?? false)
            return pluginApi?.tr("panel.checking");
        if (!(mainInstance?.mpmAvailable ?? false))
            return pluginApi?.tr("panel.installHint");
        if ((mainInstance?.activeManagerIds?.length ?? 0) === 0)
            return pluginApi?.tr("panel.noActiveManagers");
        if ((mainInstance?.hasError ?? false) && root.visibleResults.length === 0)
            return pluginApi?.tr("panel.genericError");
        return pluginApi?.tr("panel.noUpdates");
    }

    function stateDetail() {
        if (!(mainInstance?.mpmAvailable ?? false))
            return mainInstance?.mpmErrorMessage ?? "";
        if ((mainInstance?.hasError ?? false) && root.visibleResults.length === 0)
            return mainInstance?.errorMessage ?? "";
        return "";
    }
}
